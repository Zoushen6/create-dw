...
export function baseHandler(event: HandlerEnum, value: any) {
  ...
  if (value.type === 'mix' && value.mode === 'inline') {
    appStore.setProjectConfig({
      menuSetting: {
        split: true,
      },
    });
  } else if (value.type === 'horizontal' && value.mode === 'top-menu') {
    appStore.setProjectConfig({
      menuSetting: {
        split: false,
      },
...
export function handler(event: HandlerEnum, value: any): DeepPartial<ProjectConfig> {
  ...
  switch (event) {
    ...
    case HandlerEnum.MENU_EXPANDED_ALL:
      return { menuSetting: { expandAllMenu: value } };
